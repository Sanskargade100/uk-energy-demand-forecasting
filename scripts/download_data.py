...
from energy_forecasting.data.download_holidays import download_holidays
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source", choices=["demand", "weather", "holidays", "all"], default="demand"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.source in ("holidays", "all"):
        hol = download_holidays(start=args.start, end=args.end, save=save)
        logger.info("Holidays: %d dates.", len(hol))
